...
def news_review_action():
    ...
    if action == "accept":
        ...
        if not reason:
            return jsonify(errno=RETCODE.PARAMERR, errmsg="请输入拒绝原因")
        news.status = -1
        news.reason = reason
        db.session.add(news)
        db.session.commit()
...
def news_edit():
    ...
    context = {"total_page": total_page,
               "current_page": current_page, "news_list": news_dict_list}
...
def news_edit_detail():
    ...
    if not news:
        return jsonify(errno=RETCODE.NODATA, errmsg="未查询到新闻数据")
...
def news_type():
    ...
    if cid:
        ...
        if not category:
            return jsonify(errno=RETCODE.NODATA, errmsg="未查询到分类数据")
        category.name = cname
    else:
        category = Category()
        category.name = cname
